...
from foo2 import RoverOdometry, convert_to_servo, process_cmd_vel
...
class RoverBaseNode(Node):
    def __init__(self):
        ...
        self.odom_tracker = RoverOdometry(wheel_radius=0.12, wheel_base=0.47, ticks_per_rev=90)
    ...
    def control_and_read_loop(self):
        ...
        while len(self.buffer) >= 9:
            if self.buffer[0] == 0xFF:
                ...
                self.x, self.y, self.theta, self.v, self.w = self.odom_tracker.update(
                    left_ticks, right_ticks, now_sec
                )
                
                # Deadband Filter for orientation caliber (ignore 1tick quantization spike)
                if abs(self.w) < 0.95 and self.v != 0.0:
                    self.w = 0.0
